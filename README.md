...
Geometry Dash is a good game but our Platformer & Practice buttons config UI should be better, it's terribly hard to get their logic of settings and the two menus actually have tons of weird designs and bugs like:
...
### Note that:
I can't get either where RobTop's snap and split option togglers (in the Platform buttons config UI) actually take effects (smh), but in case they really do something, each of the two options is preserved as a text label button and is linked with RobTop's original toggler callback.  
Welcome to share with me if you have an idea about the options' behavior.  
