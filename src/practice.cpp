...
class $modify(PracticeOptionsLayer, UIPOptionsLayer) {
		...
		this->m_fields->radioPos = PosSignal().listen(
			[this] (int tag, CCPoint pos) -> ListenerResult {
				...
				// my own
					...
					gm->m_practicePos = pos;
					if (auto pl = PlayLayer::get())
						pl->m_uiLayer->getChildByID("checkpoint-menu")->setPosition(pos);					
				}
	...
	void handleSignal(int tag, float value) {
		...
		if (tag == -100) {
			if (!value && this->m_fields->map->getScale() == 1)
		...
		// x pos
			...
			if (m_fields->pcp) {
				if (m_fields->pcpmod) {
					m_fields->pcpmod->setSavedValue("switcherMenuPositionX", value);
					m_fields->map->getChildByID("kevadroz.practicecheckpointpermanence/switcher_menu")->setPositionX(value);					
				}
			} else {
				gm->m_practicePos.x = value;
				this->m_fields->map->placeNode(ccp(value, this->m_practiceNode->getPositionY()));
				if (auto pl = PlayLayer::get())
					pl->m_uiLayer->getChildByID("checkpoint-menu")->setPositionX(value);				
			}
		}
		// y pos
		else if (tag == 514) {
			if (m_fields->pcp) {
				if (m_fields->pcpmod) {
					m_fields->pcpmod->setSavedValue("switcherMenuPositionY", value);
					m_fields->map->getChildByID("kevadroz.practicecheckpointpermanence/switcher_menu")->setPositionY(value);
				}
			} else {
				gm->m_practicePos.y = value;	
				this->m_fields->map->placeNode(ccp(this->m_practiceNode->getPositionX(), value));
				if (auto pl = PlayLayer::get())
					pl->m_uiLayer->getChildByID("checkpoint-menu")->setPositionY(value);
			}
		}		
		// opacity
		else if (tag == 1) {
            this->valueDidChange(-29, value);
			if (auto pl = PlayLayer::get()) {
				static_cast<CCSprite*>(pl->m_uiLayer->getChildByID("checkpoint-menu")->getChildByID("add-checkpoint-button")
					->getChildByID("add-checkpoint-hint"))->setOpacity(value);
				static_cast<CCSprite*>(pl->m_uiLayer->getChildByID("checkpoint-menu")->getChildByID("remove-checkpoint-button")
					->getChildByID("remove-checkpoint-hint"))->setOpacity(value);
			}
		}
	}
